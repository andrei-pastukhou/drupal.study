(function($) {
  ...
  {
    if (settings['training3_file_rows_table']) {
      $('#training3-table-files').find("tr:gt(0)").remove();
      row_array = settings['training3_file_rows_table'];
      row_array.forEach(function (value, index, array) {
        ...
        if(Number(settings['training3_user_id']) == Number(value.data.user_id)){
          style = 'class = "tr3_user_file"';
        }
        else {
          style = '';
        }
        $('#training3-table-files > tbody:last').append('<tr ' + style + '>' +
            '<td>' + (index + 1) + '</td>' +
